...
class Drone:
    def __init__(self,x,y,z):
        ...
        self.ref_alt= round(sqrt(pow((a-3)*ref_alt_ratio,2)- pow((((a-3)*ref_alt_ratio)/sq3),2)),2) # Reference alt (drone hight when it is alone )
    ...
    def demand_neighbors_info(self,force=False): 
        ...
        if self.list_finished_update.is_set() and ( (time.time()-self.last_seen_demand_neighbors_info>10) or (force) ) : # There is no other thread doing the update 
            if force:
                print( " fORCED ")
