...
struct Point
{
    ...
    // Default constructor
    Point() : x(0),
              y(0), distance(0), angle(0), visited(false), clusterId(UNCLASSIFIED) {}

    // Constructor with x and y parameters
    Point(float xVal, float yVal) : x(xVal), y(yVal), distance(0), angle(0), visited(false), clusterId(UNCLASSIFIED) {}
};
...
void filterOutliers(std::vector<int> &clusterPoints);
Point calculateWeightedCentroid(const std::vector<int> &clusterPoints);
int calculateClusterMedoid(const std::vector<int> &clusterPoints);
float calculateDynamicOutlierThreshold(const std::vector<int> &clusterPoints);
void gather_clusters_info(std::vector<ClusterInfo> &clusters);
void gather_clusters_info_old(std::vector<ClusterInfo> &clusters);
...
void detect_objects_clustering(std::vector<ClusterInfo> &clusters)
{
    ...
    std::vector<ClusterInfo> clusters_cpoy = clusters;
    Serial.println("old");
    gather_clusters_info_old(clusters_cpoy);
    printClustersInfo(clusters_cpoy, 1);
...
void filterOutliers(std::vector<int> &clusterPoints)
{
    float outlierThreshold = calculateDynamicOutlierThreshold(clusterPoints);

    clusterPoints.erase(std::remove_if(clusterPoints.begin(), clusterPoints.end(),
                                       [&](int pointIdx)
                                       {
                                           float sumDist = 0.0;
                                           for (int j : clusterPoints)
                                           {
                                               sumDist += calculateDistance(points[pointIdx], points[j]);
                                           }
                                           float avgDist = sumDist / clusterPoints.size();
                                           return avgDist > outlierThreshold;
                                       }),
                        clusterPoints.end());
}
...
Point calculateWeightedCentroid(const std::vector<int> &clusterPoints)
{
    ...
    {
        float weight = 1.0; // Define your weighting criterion here
        sumX += points[idx].x * weight;
        sumY += points[idx].y * weight;
        totalWeight += weight;
    }
    return Point{sumX / totalWeight, sumY / totalWeight};
}
...
int calculateClusterMedoid(const std::vector<int> &clusterPoints)
{
    ...
    for (int i : clusterPoints)
    {
        ...
        for (int j : clusterPoints)
        {
            sumDistance += calculateDistance(points[i], points[j]);
        }
        float avgDistance = sumDistance / clusterPoints.size();
...
float calculateDynamicOutlierThreshold(const std::vector<int> &clusterPoints)
{
    ...
    float Q1 = distances[q1Idx];
    float Q3 = distances[q3Idx];
    float IQR = Q3 - Q1;

    return 1.5 * IQR; // 1.5 is a common factor for outlier detection
}
...
void gather_clusters_info(std::vector<ClusterInfo> &clusters)
{
    ...
    // Post-processing for each cluster
    for (int i = 1; i < MAX_CLUSTERS; ++i)
    {
        if (clusterData[i].count > 0)
        {
            // Collect all points of the current cluster
            ...
            for (int j = 0; j < numPoints; ++j)
            {
                if (points[j].clusterId == i)
                {
                    ...
                    Serial.print(i);
                    Serial.print("Distance");
                    Serial.println(sqrt(pow(points[j].x, 2), pow(points[j].y, 2)));
                    clusterPoints.push_back(j);
                }
            }

            // Outlier filtering
            filterOutliers(clusterPoints);

            // Calculate weighted centroid or medoid
            // Point centroid = calculateWeightedCentroid(clusterPoints);
            int medoidIndex = calculateClusterMedoid(clusterPoints);
            Point centroid = points[medoidIndex]; // Use this line instead if using medoid

            clusterData[i].centroidX = centroid.x;
            clusterData[i].centroidY = centroid.y;
            clusterData[i].centerDistance = sqrt(pow(clusterData[i].centroidX, 2) + pow(clusterData[i].centroidY, 2));
            clusters.push_back(clusterData[i]);
        }
    }
