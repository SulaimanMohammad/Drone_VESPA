...
std::vector<Object> objects_info;
...
void loop()
{
    ...
    std::vector<Object> detected_object = define_objects(clustersSweep1, clusterMap);
    printObjects(detected_object);
    print_position_from_sensor(clustersSweep1);
...
void detect_objects_clustering(std::vector<ClusterInfo> &clusters, std::map<int, std::vector<Point>> &clusterMap)
{
    ...
    Serial.print("Number of clusters detected from main: ");
    Serial.println(numberOfClusters);
...
void gather_clusters_info(std::vector<ClusterInfo> &clusters)
{
    ...
    for (int i = 0; i <= numberOfClusters; ++i)
    {
        ...
        Serial.println(clusterData[i].count);
