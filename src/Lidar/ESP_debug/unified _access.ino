...
{
    numPoints = 0;
    int step = (start_point < end_point) ? FOV : -FOV;
    ...
    for (int pan_servoPosition = start_point; (step > 0) ? (pan_servoPosition <= end_point) : (pan_servoPosition >= end_point); pan_servoPosition += step)
    {
        ...
        int tiltStep = (pan_servoPosition % (2 * FOV) == 0) ? FOV : -FOV; // Alternate tilt direction every pan step
        int tiltStart = (tiltStep > 0) ? tilt_start_range : tilt_end_range;
        int tiltEnd = (tiltStep > 0) ? tilt_end_range : tilt_start_range;

        for (int tilt_servoPosition = tiltStart; (tiltStep > 0) ? (tilt_servoPosition <= tiltEnd) : (tilt_servoPosition >= tiltEnd); tilt_servoPosition += tiltStep)
        {
