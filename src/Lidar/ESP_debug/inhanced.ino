...
int FOV = 3;              // Field of View of the sensor
int pan_start_range = 20;
int pan_end_range = 120;
int tilt_start_range = 20;
int tilt_end_range = 120;

int object_FOV = (FOV * 3); // Field of object detection (enough angles to detect object)
int pan_active_range = (pan_end_range - pan_start_range);
int tilt_active_range = (tilt_end_range - tilt_start_range);

int MAX_POINTS(((pan_active_range / object_FOV) + 1) * ((tilt_active_range / object_FOV) + 1));
...
float calculateDistance(const Point &p1, const Point &p2)
{
    ...
    float scaledPanDiff = panDiff * 0.3;
    float scaledTiltDiff = tiltDiff * 0.7;
...
void adjustEpsilonBasedOnFeedback(int const Elbow_value, int const Knee_value)
{
    ...
    noisePoints = estimateNoisePoints(std::min(Elbow_value, Knee_value));
    Serial.print("\n min, noisePoints: ");
    Serial.println(noisePoints);
...
void gather_clusters_info(std::vector<ClusterInfo> &clusters)
{
    numberOfClusters = countUniqueClusters(); // Get the number of clusters
